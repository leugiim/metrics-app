<script lang="ts">
	...
	const NUMBER_OF_PAGINATION_PAGES = 5;
	...
	const calcPage = () => {
		...
		const end = Math.min(start + ITEMS_PER_PAGE, filteredMetrics.length);
		pageHelper = { start, end, length: filteredMetrics.length };
		paginatedMetrics = filteredMetrics.slice(start, end);
	};
	const paginateMetrics = () => {
		pages = [];
		numPages = Math.ceil(filteredMetrics.length / ITEMS_PER_PAGE);
		if (activePage >= numPages) activePage = 0;
		const startPage = Math.max(activePage - NUMBER_OF_PAGINATION_PAGES, 0);
		const endPage = Math.min(activePage + NUMBER_OF_PAGINATION_PAGES, numPages);
